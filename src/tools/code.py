        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            ...
            try:
                ...
                if 0 < node.lineno <= len(lines):
                    ...
                    defs.append(
                        f"| Error: Invalid line number {node.lineno} for a definition in {file_path.name}"
                    )

            except IndexError:  # Should not happen if node.lineno is valid
                defs.append(
                    f"| Error: Could not retrieve line for a definition in {file_path.name}"
                )
    return defs
...
def _javascript_top_level_defs(file_path: Path) -> List[str]:
    ...
    for line in lines:
        ...
        if (
            pattern_func.match(stripped)
            or pattern_class.match(stripped)
            or pattern_arrow.match(stripped)
        ):
...
class ListCodeDefinitionNamesTool(Tool):
    ...
    def parameters_schema(self) -> Dict[str, str]:
        return {"path": "The path of the directory to list top level source code definitions for."}
    ...
    def execute(self, params: Dict[str, Any], agent_tools_instance: Any) -> str:
        ...
        try:
            ...
            for entry in sorted(abs_dir_path.iterdir()):
                if entry.is_file() and entry.suffix in suffix_map:
                    ...
                    if defs:
                        ...
                        if any(
                            not d.startswith("Error:") and not d.startswith("| Error:")
                            for d in defs
                        ):
                            found_defs = True
            ...
            if not found_defs and any(
                "Error:" in d for item in output_structure for d in item["definitions"]
            ):
                # Only errors were found during parsing of some files
                return json.dumps(
                    {
                        "results": output_structure,
                        "message": "Found source files, but encountered errors while parsing definitions.",
                    }
                )
            ...
            return json.dumps(
                {
                    "results": output_structure,
                    "message": "Successfully listed code definitions."
                    if found_defs
                    else "No definitions found in source files.",
                }
            )
...
def list_code_definition_names(
    directory_path: str, agent_tools_instance: Any = None
) -> str:  # Renamed parameter
    tool = ListCodeDefinitionNamesTool()
    result_str = tool.execute(
        {"path": directory_path}, agent_tools_instance=agent_tools_instance
    )  # Pass updated param
    ...
    if "message" in data and (
        data["message"] == "No supported source files found in the directory."
        or data["message"] == "No definitions found in source files."
    ):
        return "No source code definitions found."
    ...
    if "results" in data:
        ...
        for file_info in data["results"]:
            ...
            output_lines.append(
                "|----"
            )  # Test expects this separator at the end of each file block
        return "\n".join(output_lines)
