...
import defusedxml.ElementTree as ET  # Replaced for security
...
def parse_assistant_message(message: str) -> List[AssistantMessageContent]:
    ...
    try:
        ...
        if stripped_message:
            ...
            if stripped_message.startswith("<") and stripped_message.endswith(">"):
                result.append(
                    TextContent(
                        content=f"{MALFORMED_TOOL_PREFIX}Could not parse XML: {stripped_message}",
                        partial=False,
                    )
                )
            else:
    ...
    for element in root:  # element is a direct child of <root_message_wrapper>
        if (
            element.tag in TOOL_USE_NAMES
        ):  # Handles direct tool calls like <read_file>...</read_file>
            ...
            element.tag == "tool_use"
        ):  # Handles wrapped tool calls like <tool_use><tool_name>...</tool_name>...</tool_use>
            tool_name_element = element.find("tool_name")
            original_xml_snippet = ET.tostring(element, encoding="unicode", method="xml").strip()
            if tool_name_element is None or not (
                tool_name_element.text and tool_name_element.text.strip()
            ):
                # Malformed wrapped tool_use (missing tool_name)
                result.append(
                    TextContent(
                        content=f"{MALFORMED_TOOL_PREFIX}Missing <tool_name> in <tool_use> block: {original_xml_snippet}"
                    )
                )
            else:
            ...
            element_copy = copy.deepcopy(element)
            element_copy.tail = None
            original_xml_snippet = ET.tostring(
                element_copy, encoding="unicode", method="xml"
            ).strip()
            if original_xml_snippet:
                result.append(
                    TextContent(
                        content=f"{MALFORMED_TOOL_PREFIX}Unrecognized or malformed tag: {original_xml_snippet}"
                    )
                )
