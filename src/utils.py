...
def revert_to_commit(cwd: str, commit_hash: str) -> bool:
    ...
    try:
        ...
        result = subprocess.run(cmd, cwd=cwd, check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode == 0:
            logging.info(f"Successfully reverted to commit {commit_hash} in {cwd}")
            return True
        else:
            logging.error(f"Failed to revert to commit {commit_hash} in {cwd}: {result.stderr.decode().strip()}")
            return False
    except Exception as e:
        logging.error(f"An unexpected error occurred while reverting to commit {commit_hash} in {cwd}: {e}")
        return False
...
def revert_to_state_before_commit(cwd: str, commit_hash: str) -> bool:
    ...
    try:
        ...
        result = subprocess.run(cmd, cwd=cwd, check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode == 0:
            logging.info(f"Successfully reverted to state before commit {commit_hash} (to {commit_to_revert_to}) in {cwd}")
            return True
        else:
            logging.error(f"Failed to revert to state before commit {commit_hash} in {cwd}: {result.stderr.decode().strip()}")
            return False
    except Exception as e:
        logging.error(f"An unexpected error occurred while reverting to state before {commit_hash} in {cwd}: {e}")
        return False
...
def get_initial_commit(cwd: str) -> Optional[str]:
    ...
    try:
        ...
        if "does not have any commits yet" in e.stderr.lower() or "bad default revision 'head'" in e.stderr.lower():
