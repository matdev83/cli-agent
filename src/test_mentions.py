...
class TestFileMentionRegex(unittest.TestCase):
    def test_unix_paths(self):
        ...
        self.assertEqual(
            extract_file_mentions("@/path/to/another_file.py"), ["/path/to/another_file.py"]
        )
        self.assertEqual(
            extract_file_mentions("@/path/with-hyphen/file.ts"), ["/path/with-hyphen/file.ts"]
        )

    def test_windows_paths(self):
        self.assertEqual(
            extract_file_mentions("@C:\\path\\to\\file.txt"), ["C:\\path\\to\\file.txt"]
        )
        self.assertEqual(
            extract_file_mentions("@D:\\path\\to\\another_file.py"),
            ["D:\\path\\to\\another_file.py"],
        )
        self.assertEqual(
            extract_file_mentions("@C:\\path\\with-hyphen\\file.ts"),
            ["C:\\path\\with-hyphen\\file.ts"],
        )

    def test_paths_with_spaces(self):
        self.assertEqual(
            extract_file_mentions("@/path/to a file with spaces.txt"),
            ["/path/to a file with spaces.txt"],
        )
        self.assertEqual(
            extract_file_mentions("@C:\\path\\to a file with spaces.docx"),
            ["C:\\path\\to a file with spaces.docx"],
        )
    ...
    def test_other_mentions(self):
        ...
        self.assertEqual(
            extract_file_mentions("Look at @/file.txt for details."), ["/file.txt for details."]
        )
    ...
    def test_regex_direct(self):
        ...
        match_windows = FILE_MENTION_REGEX.search(
            "Refer to @C:\\Users\\MyUser\\file with space.txt for more."
        )
        self.assertIsNotNone(match_windows)
        if match_windows:  # Satisfy type checker
            # Acknowledging that "for more." will be captured.
            self.assertEqual(
                match_windows.group(1), "C:\\Users\\MyUser\\file with space.txt for more."
            )
