...
# Dummy AgentToolsInstance class for testing
...
def test_browser_action_missing_action_parameter():
    tool = BrowserActionTool()
    agent_tools_instance = MockAgentToolsInstance()
    result_str = tool.execute({}, agent_tools_instance=agent_tools_instance)
...
def test_browser_action_launch_success(mock_sync_playwright, tmp_path):
    agent_tools_instance = MockAgentToolsInstance(cwd=str(tmp_path))
    tool = BrowserActionTool()
    ...
    assert agent_tools_instance.playwright_sync_instance == mock_ps_instance
    assert agent_tools_instance.playwright_browser_context == mock_context_instance
    assert agent_tools_instance.playwright_page == mock_page_instance
...
def test_browser_action_close_success(mock_sync_playwright_unused, tmp_path):
    ...
    # Setup mock objects in agent_tools_instance to simulate an open browser
    mock_context_to_close = MagicMock(name="ContextToClose")
    mock_sync_instance_to_stop = MagicMock(name="SyncInstanceToStop")
    agent_tools_instance.playwright_browser_context = mock_context_to_close
    agent_tools_instance.playwright_sync_instance = mock_sync_instance_to_stop
    agent_tools_instance.playwright_page = MagicMock() # Page isn't directly used by close, but good to have
    ...
    assert agent_tools_instance.playwright_sync_instance is None
    assert agent_tools_instance.playwright_browser_context is None
    assert agent_tools_instance.playwright_page is None


def test_browser_action_close_when_nothing_open():
    tool = BrowserActionTool()
    agent_tools_instance = MockAgentToolsInstance() # No browser state
    params = {"action": "close"}
    result_str = tool.execute(params, agent_tools_instance=agent_tools_instance)
    result = json.loads(result_str)
...
def test_actions_require_page_not_launched():
    tool = BrowserActionTool()
    agent_tools_instance = MockAgentToolsInstance() # No page set
...
def test_browser_action_click_success(mock_sync_playwright_unused):
    ...
    agent_tools_instance = MockAgentToolsInstance()
    mock_page = MagicMock()
    agent_tools_instance.playwright_page = mock_page
...
def test_browser_action_click_missing_coordinate():
    tool = BrowserActionTool()
    agent_tools_instance = MockAgentToolsInstance()
    agent_tools_instance.playwright_page = MagicMock() # Page exists
    params = {"action": "click"}
    result_str = tool.execute(params, agent_tools_instance=agent_tools_instance)
    result = json.loads(result_str)
    assert result["status"] == "error"
...
def test_browser_action_click_invalid_coordinate():
    tool = BrowserActionTool()
    agent_tools_instance = MockAgentToolsInstance()
    agent_tools_instance.playwright_page = MagicMock()
...
def test_browser_action_launch_cleans_up_old_state(mock_sync_playwright, tmp_path):
    ...
    # Pre-populate agent_tools_instance with old mocks
    old_mock_ps_instance = MagicMock(name="old_ps_instance")
    old_mock_context_instance = MagicMock(name="old_context_instance")
    agent_tools_instance.playwright_sync_instance = old_mock_ps_instance
    agent_tools_instance.playwright_browser_context = old_mock_context_instance
    agent_tools_instance.playwright_page = MagicMock(name="old_page_instance")
...
def test_browser_action_generic_error_handling(mock_get_page):
    ...
    # No need to set agent_tools_instance.playwright_page if _get_playwright_page itself is mocked to fail
    # agent_tools_instance.playwright_page = mock_page_for_error

    # Store the mocks for context and sync_instance that might be on agent_tools_instance
    ...
    agent_tools_instance.playwright_browser_context = mock_context_on_agent
    agent_tools_instance.playwright_sync_instance = mock_sync_on_agent
    ...
    assert agent_tools_instance.playwright_page is None # State should be cleared by the handler
    assert agent_tools_instance.playwright_browser_context is None
    assert agent_tools_instance.playwright_sync_instance is None
